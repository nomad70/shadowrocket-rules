...
PlayfulSR V2.4.1 对 AI 服务采用分级保护：Claude 使用严格失败关闭；ChatGPT/Gemini 默认使用美国可信出口，也允许手动固定美国 ISP/DC。三者都不能回退到本地网络。
...
3. 关闭“睡眠时断开”。
4. 为使用的节点开启 UDP 转发；若节点不支持，V2.4.1 会拒绝 UDP，而不是本地直连。支持 UDP 的普通代理流量仍可正常使用 QUIC。
...
如果启用浏览器语音、视频或其他 WebRTC 功能，还可以在 Shadowrocket 设置中启用“禁用 STUN”进行更严格的隐私保护；这可能导致 ChatGPT 语音、视频通话或其他 WebRTC 应用不可用，因此 V2.4.1 不默认全局启用。
